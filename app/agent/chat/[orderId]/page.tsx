...
export default function AgentChatThreadPage() {
  ...
  const send = async () => {
    ...
    const res = await fetch("/api/order-messages", {
      ...
      body: JSON.stringify({ orderId: String(params.orderId), senderRole: "agent", message }),
    });
    const data = await res.json();
  ...
  if (loading) return <SidebarShell role="agent"><LoadingCard text="Loading order chat..." /></SidebarShell>;
  if (!order) return <SidebarShell role="agent"><GlassCard className="p-10 text-center">Order not found.</GlassCard></SidebarShell>;

  return (
    <SidebarShell role="agent">
      <PageHeader title={`Order ${order.id}`} subtitle="Keep the chat open, answer the player directly and continue the recharge conversation without leaving the order thread." />
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <GlassCard className="p-6 md:p-8">
          <div className="space-y-3">
            {(order.messages || []).map((msg: any, idx: number) => (
              <div key={idx} className={`max-w-[85%] rounded-3xl px-4 py-3 text-sm ${msg.senderRole === "agent" ? "ml-auto bg-cyan-400/15 text-white" : "bg-white/5 text-white/80"}`}>
                <p className="font-semibold capitalize text-white/90">{msg.senderRole}</p>
                <p className="mt-1 whitespace-pre-wrap">{msg.message}</p>
                <p className="mt-2 text-[11px] text-white/40">{new Date(msg.created_at).toLocaleString()}</p>
              </div>
            ))}
            {!order.messages?.length ? <div className="text-white/55">No messages yet.</div> : null}
          </div>
          <div className="mt-5 space-y-3">
            <TextArea rows={4} value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Write your message to the player..." />
        ...
        <GlassCard className="p-6 md:p-8">
          ...
          <div className="mt-5 grid gap-3 text-sm text-white/65">
            <p>Player: <span className="font-semibold text-white">{order.playerEmail}</span></p>
            ...
            <p>Method: <span className="font-semibold text-white">{order.payment_method_name || "—"}</span></p>
          </div>
        </GlassCard>
      </div>
    </SidebarShell>
