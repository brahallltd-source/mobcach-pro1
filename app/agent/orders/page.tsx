...
import { EmptyState, GlassCard, LoadingCard, PageHeader, SidebarShell, StatCard, StatusBadge, TextField } from "@/components/ui";
...
export default function AgentOrdersPage() {
  ...
  const newOrders = useMemo(() => orders.filter((item) => ["proof_uploaded", "pending_payment"].includes(item.status)).length, [orders]);
  const waitingPlayer = useMemo(() => orders.filter((item) => item.status === "agent_approved_waiting_player").length, [orders]);
  const completed = useMemo(() => orders.filter((item) => item.status === "completed").length, [orders]);

  const filteredOrders = useMemo(() => {
    let base = orders;
    if (filter === "new") base = orders.filter((item) => ["proof_uploaded", "pending_payment"].includes(item.status));
    if (filter === "waiting") base = orders.filter((item) => item.status === "agent_approved_waiting_player");
    if (filter === "completed") base = orders.filter((item) => item.status === "completed");
    ...
    return base.filter((item) =>
      ...
      String(item.gosport365_username || "").toLowerCase().includes(q) ||
      String(item.payment_method_name || "").toLowerCase().includes(q) ||
      item.id.toLowerCase().includes(q)
    );
  }, [orders, filter, query]);

  if (loading) return <SidebarShell role="agent"><LoadingCard text="Loading orders..." /></SidebarShell>;
              ...
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <p className="mt-2 text-sm text-white/55">
                    {order.payment_method_name || "Method pending"} • {order.gosport365_username || "Username pending"}
                  </p>
