...
import { DangerButton, GlassCard, LoadingCard, NavPill, PageHeader, PrimaryButton, SelectField, SidebarShell, StatCard, TextField } from "@/components/ui";

type User = { id: string; email: string; role: string; agentId?: string };
type Agent = { full_name?: string; email: string; phone?: string };
type Profile = { agentId: string; response_minutes: number; rating?: number; trades_count?: number };
...
export default function AgentSettingsPage() {
  ...
  const [form, setForm] = useState({ email: "", phone: "", response_minutes: 15 });

  const load = async (agentId: string) => {
    const res = await fetch(`/api/agent/settings?agentId=${encodeURIComponent(agentId)}`, { cache: "no-store" });
    ...
    setForm({
      ...
      response_minutes: Number(data.profile?.response_minutes || 15),
    });
  };
  ...
  if (loading || !user) return <SidebarShell role="agent"><LoadingCard text="Loading settings..." /></SidebarShell>;
  ...
  return (
    <SidebarShell role="agent">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <StatCard label="Rating" value={`${profile?.rating || 0}%`} hint="Visible in Select Agent and Achat" />
        <StatCard label="Trades" value={String(profile?.trades_count || 0)} hint="Public volume used for trust" />
      </div>
      <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
        <GlassCard className="p-6 md:p-8">
          <h2 className="text-2xl font-semibold">Personal information</h2>
          <p className="mt-2 text-sm leading-6 text-white/60">Update your email, phone and task execution promise. These values are reflected throughout the player-facing purchase flow.</p>
          <div className="mt-6 space-y-4">
            <TextField value={agent?.full_name || ""} disabled placeholder="Full name" />
            <TextField value={form.email} onChange={(e) => setForm((prev) => ({ ...prev, email: e.target.value }))} placeholder="Email" />
            <TextField value={form.phone} onChange={(e) => setForm((prev) => ({ ...prev, phone: e.target.value }))} placeholder="Phone number" />
            <SelectField value={String(form.response_minutes)} onChange={(e) => setForm((prev) => ({ ...prev, response_minutes: Number(e.target.value) }))}>
              {EXECUTION_TIME_OPTIONS.map((minutes) => <option key={minutes} value={minutes}>{minutes} min</option>)}
            </SelectField>
            <PrimaryButton onClick={save} disabled={saving} className="w-full">{saving ? "Saving..." : "Save settings"}</PrimaryButton>
          </div>
        </GlassCard>
        <div className="space-y-6">
          ...
          <GlassCard className="p-6 md:p-8">
            ...
            <p className="mt-2 text-sm leading-6 text-white/60">Sign out from the agent workspace when you finish your review and payout operations.</p>
            <div className="mt-5">
