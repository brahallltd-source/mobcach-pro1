...
type AppRecord = { id: string; full_name?: string; first_name?: string; last_name?: string; email: string; phone?: string; note?: string; status: string; created_at?: string };
...
export default function PlayerBecomeAgentPage() {
  ...
  const load = async (userEmail: string) => {
    const res = await fetch(`/api/player/become-agent?email=${encodeURIComponent(userEmail)}`, { cache: "no-store" });
  ...
  const submit = async () => {
    setSaving(true);
    try {
      const res = await fetch("/api/player/become-agent", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, phone, email, note }) });
      const data = await res.json();
      if (!res.ok) { alert(data.message || "Failed to submit application"); setSaving(false); return; }
      setCurrent(data.application || null);
        ...
        {current ? (
          <GlassCard className="p-6">
            ...
            <div className="mt-5 grid gap-3 text-sm text-white/70">
              <p><span className="text-white/45">Name:</span> {current.full_name || `${current.first_name || ""} ${current.last_name || ""}`.trim()}</p>
