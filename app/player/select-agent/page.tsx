...
type AgentRow = { agentId: string; display_name: string; email: string; online: boolean; country: string; rating: number; trades_count: number; response_minutes: number; min_limit: number; max_limit: number; available_balance: number; verified: boolean; featured: boolean; trusted?: boolean; fast?: boolean; success_rate?: number; last_activity?: string; bank_methods: string[]; supported_assets: string[] };
...
export default function PlayerSelectAgentPage() {
  ...
  useEffect(() => { load().finally(() => setLoading(false)); }, [country, method, amount, time]);
  ...
  const availableMethods = useMemo(() => {
    ...
    agents.forEach((agent) => agent.bank_methods.forEach((item) => set.add(item)));
  ...
  if (loading) return <SidebarShell role="player"><LoadingCard text="Loading agent desk..." /></SidebarShell>;

  return (
    <SidebarShell role="player">
      <PageHeader title={t("selectAgent")} subtitle="Compare agents like a pro desk: speed, trust score, supported banks, live availability and realistic payment limits optimized for mobile first selection." />
      ...
      <GlassCard className="overflow-hidden p-4 md:p-6">
        <div className="grid gap-3 md:grid-cols-4">
          ...
          <select value={method} onChange={(e) => setMethod(e.target.value)} className="w-full rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-white outline-none">
            {availableMethods.map((item) => <option key={item} value={item}>{item}</option>)}
          </select>
          <select value={time} onChange={(e) => setTime(e.target.value)} className="w-full rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-white outline-none">
            {[0, 15, 30, 45, 60, 120, 180].map((value) => <option key={value} value={value}>{value === 0 ? t("all") : `${value} min`}</option>)}
          </select>
        </div>
      </GlassCard>
      <div className="space-y-4">
              ...
              <div>
                <div className="flex items-start justify-between gap-4">
                  <div className="flex min-w-0 items-center gap-3">
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-white/10 text-lg font-semibold">{agent.display_name.slice(0, 1)}</div>
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <h3 className="truncate text-2xl font-semibold">{agent.display_name}</h3>
                        {agent.online ? <span className="inline-block h-2.5 w-2.5 rounded-full bg-emerald-400" /> : <span className="inline-block h-2.5 w-2.5 rounded-full bg-white/20" />}
                        {agent.verified ? <span className="rounded-full bg-emerald-400/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.2em] text-emerald-200">verified</span> : null}
                      </div>
                      <p className="text-sm text-white/50">{agent.country} • {agent.online ? t("online") : t("offline")}</p>
                    </div>
                  </div>
                  {agent.featured ? <div className="rounded-full bg-amber-300/15 px-3 py-1 text-xs font-semibold uppercase tracking-[0.24em] text-amber-100">Featured</div> : null}
                </div>
                ...
                <div className="mt-5 rounded-3xl border border-white/10 bg-black/20 p-4">
                  <div className="flex items-center gap-2 text-white/55"><WalletCards size={16} /> Supported methods</div>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {agent.bank_methods.map((item) => <span key={item} className="rounded-full border border-white/10 bg-white/5 px-3 py-2 text-xs font-semibold text-white/75">{item}</span>)}
                  </div>
                </div>
              </div>
              <div className="rounded-[30px] border border-white/10 bg-black/20 p-5">
                <p className="text-sm text-white/45">{t("limit")}</p>
                <p className="mt-2 text-3xl font-semibold">{agent.min_limit.toLocaleString()} - {agent.max_limit.toLocaleString()} MAD</p>
                <p className="mt-3 text-sm text-white/60">{t("available")}: {agent.available_balance.toLocaleString()} MAD</p>
                <p className="mt-2 text-sm text-white/60">Assets: {agent.supported_assets.join(" • ")}</p>
                ...
                <PrimaryButton onClick={() => chooseAgent(agent.agentId)} disabled={selectingAgentId === agent.agentId} className="mt-6 w-full">
        ...
        {!agents.length ? <GlassCard className="p-10 text-center text-white/60">{t("noOffers")}</GlassCard> : null}
      </div>
    </SidebarShell>
  );
}
