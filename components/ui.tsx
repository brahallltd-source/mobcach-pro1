...
import {
  Bell, CircleDollarSign, LayoutDashboard, LifeBuoy, MessageCircle, Package, Users,
  Languages, Settings, Wallet, Menu, X, Zap, LogOut, Sparkles, ShoppingCart, UserRound, ShieldCheck, CreditCard, ShieldAlert
} from "lucide-react";
import { LanguageSwitcher, useLanguage } from "@/components/language";

export function GlassCard({ children, className = "" }: { children: ReactNode; className?: string }) {
  return <div className={clsx("rounded-[28px] border border-white/10 bg-white/[0.06] backdrop-blur-xl shadow-glass", className)}>{children}</div>;
}
  ...
  return (
    <GlassCard className="overflow-hidden p-6 md:p-8">
      ...
      <div className="relative flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <div className="inline-flex rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.34em] text-cyan-200">MobCash Pro</div>
  ...
  return (
    <main dir={dir} className="min-h-screen bg-hero px-4 py-6 text-white md:px-6">
      <div className="mx-auto mb-4 flex max-w-7xl items-center justify-between gap-4">
        <Link href="/" className="inline-flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-3 text-sm font-semibold text-white/85 backdrop-blur-xl transition hover:bg-white/[0.08]">
          <span className="rounded-xl bg-cyan-400/15 p-2 text-cyan-200"><Languages size={16} /></span>
          MobCash Pro
        </Link>
        <LanguageSwitcher />
      </div>
        ...
        <aside className="hidden w-72 shrink-0 lg:block">
          <GlassCard className="sticky top-5 p-4">
            <div className="mb-4 flex items-center justify-between">
              <div className="text-lg font-semibold">MobCash Pro</div>
              <LanguageSwitcher />
            </div>
            <div className="space-y-2">
              {nav.map((item) => {
                ...
                return (
                  <Link key={item.href} href={item.href} className={clsx("flex items-center gap-3 rounded-2xl px-4 py-3.5 text-sm font-semibold transition", active ? "bg-white text-slate-950" : "bg-white/5 text-white/75 hover:bg-white/10 hover:text-white")}>
                    <Icon size={16} />
                    {item.label}
                  </Link>
                );
            ...
            <button onClick={logout} className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-semibold text-white/80 transition hover:bg-white/10">
              <LogOut size={16} />
              Logout
            </button>
          </GlassCard>
              ...
              <GlassCard className="max-h-[90vh] overflow-y-auto p-4">
                ...
                <div className="space-y-2">
                  {nav.map((item) => {
                    ...
                    const Icon = item.icon;
                    return (
                      <Link key={item.href} href={item.href} onClick={() => setOpen(false)} className={clsx("flex items-center gap-3 rounded-2xl px-4 py-3.5 text-sm font-semibold transition", active ? "bg-white text-slate-950" : "bg-white/5 text-white/75 hover:bg-white/10 hover:text-white")}>
                        <Icon size={16} />
                        {item.label}
                      </Link>
                    );
...
export function TextField(props: InputHTMLAttributes<HTMLInputElement>) {
  return <input {...props} className={clsx("w-full rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-white outline-none placeholder:text-white/30", props.className)} />;
}
...
export function TextArea(props: TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return <textarea {...props} className={clsx("w-full rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-white outline-none placeholder:text-white/30", props.className)} />;
}
...
export function StatCard({ label, value, hint }: { label: string; value: string; hint?: string }) {
  return <GlassCard className="p-5"><p className="text-sm text-white/50">{label}</p><p className="mt-2 text-3xl font-semibold">{value}</p>{hint ? <p className="mt-2 text-sm text-white/45">{hint}</p> : null}</GlassCard>;
}
