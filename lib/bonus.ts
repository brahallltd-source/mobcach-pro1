...
export function getTaskProgress(agentId: string): AgentTaskProgress {
  const tasks = readJsonArray<AgentTaskProgress>(taskProgressPath);
  const existing = tasks.find(
    (item) => String(item.agentId) === String(agentId) && item.task_key === TASK_KEY
  );
...
export function getPendingBonuses(agentId: string) {
  return readJsonArray<PendingBonus>(pendingBonusesPath).filter(
    (item) => String(item.agentId) === String(agentId) && item.status === "pending"
  );
}

function addPendingBonus(
  agentId: string,
  source: PendingBonus["source"],
  source_ref: string,
  amount: number,
  meta?: Record<string, unknown>
) {
  ...
  const duplicate = pending.find(
    (item) =>
      String(item.agentId) === String(agentId) &&
      item.source === source &&
...
export function applyPendingBonusesToRecharge(agentId: string, adminEmail?: string) {
  ...
  if (total <= 0) {
    return { totalApplied: 0, items: [] as PendingBonus[] };
  }

  const next = pending.map((item) =>
    target.some((p) => p.id === item.id)
      ? {
          ...item,
          status: "applied" as const,
          applied_at: nowIso(),
          meta: { ...(item.meta || {}), adminEmail },
        }
      : item
  );
  ...
  creditWallet(agentId, total, "bonus_pending_apply_on_recharge", {
    adminEmail,
    count: target.length,
  });
...
export function recordOrderActivity(agentId: string, amount: number, orderId: string) {
  ...
  log(agentId, "order_activity_recorded", {
    amount,
    orderId,
...
export function getLevelClaims(agentId: string) {
  return readJsonArray<BonusLevelClaim>(levelClaimsPath).filter(
    (item) => String(item.agentId) === String(agentId)
  );
}
...
export function unlockLevelReward(agentId: string, level: number) {
  ...
  if (
    claims.some(
      (item) => String(item.agentId) === String(agentId) && item.level === def.level
    )
  ) {
    throw new Error("Level reward already claimed");
  }

  if (profile.volume < def.target) {
    throw new Error("Level target not reached");
  }

  const claim: BonusLevelClaim = {
    id: uid("level-claim"),
    agentId: String(agentId),
    level: def.level,
    reward: def.reward,
  ...
  addPendingBonus(agentId, "level", String(def.level), def.reward, {
    target: def.target,
  });

  log(agentId, "level_reward_unlocked", {
    level: def.level,
    reward: def.reward,
  });
...
export function unlockEnergyReward(agentId: string) {
  ...
  if (profile.energy < ENERGY_TARGET) {
    throw new Error("Energy not full yet");
  }
  ...
  const existing = pending.find(
    (item) =>
      String(item.agentId) === String(agentId) &&
      item.source === "energy" &&
      item.source_ref === String(profile.last_order_at || "energy")
  );

  if (existing) {
    throw new Error("Energy reward already claimed");
  }

  addPendingBonus(
    agentId,
    "energy",
    String(profile.last_order_at || uid("energy")),
    ENERGY_REWARD,
...
export function claimTaskReward(agentId: string) {
  ...
  addPendingBonus(agentId, "task", `${task.task_key}-${task.cycle}`, task.reward, {
    target: task.target,
    cycle: task.cycle,
  });
  ...
  log(agentId, "task_reward_claimed", {
    task: task.task_key,
    reward: task.reward,
    cycle: task.cycle,
  });
...
export function getReferralRows(agentId: string) {
  return readJsonArray<Referral>(referralsPath).filter(
    (item) => String(item.referred_by_agent_id) === String(agentId)
  );
}
...
export function createReferral(
  payload: Omit<
    Referral,
    "id" | "created_at" | "updated_at" | "reward_status" | "rewarded_order_id"
  >
) {
...
export function rewardReferralOnFirstOrder(
  playerEmail: string,
  orderId: string,
  amount: number
) {
  ...
  const index = referrals.findIndex(
    (item) => item.player_email.toLowerCase() === String(playerEmail).toLowerCase()
  );
  ...
  if (referral.reward_status !== "none") {
    return null;
  }
  ...
  referrals[index] = {
    ...referral,
    first_order_reward_amount: rewardAmount,
    reward_status: "pending",
    rewarded_order_id: orderId,
    updated_at: nowIso(),
  };

  writeJsonArray(referralsPath, referrals);

  addPendingBonus(referral.referred_by_agent_id, "referral", orderId, rewardAmount, {
    playerEmail,
    percent: 2,
  });

  log(referral.referred_by_agent_id, "referral_reward_pending", {
    playerEmail,
    orderId,
    rewardAmount,
  });
